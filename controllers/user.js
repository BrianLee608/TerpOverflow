...
exports.userGet = function(req, res, next) {
  User.findOne({ displayName: req.params.username }, function(err, user) {
    if (err) return next(err);
    if (!user) return res.render('error', {error: 'User not found'});

    Question.find({ author: user._id }, function(err, questions) {
      if (err) return next(err);

      res.render('account/profile', {user, questions});
    })
  })
}
  ...
  async.waterfall([
    ...
    function(token, user, done) {
      ...
      var mailOptions = {
        ...
        subject: '✔ Reset your password on Terp Overflow',
